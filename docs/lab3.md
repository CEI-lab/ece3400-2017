...
The VGA driver module on the FPGA generates the necessary VGA color signals (8-bit RGB color: 3 bits for red, 3 bits for green, and 2 bits for blue) and synchronization signal. It also outputs the x- and y-coordinates of the next pixel that is needed which you will use to determine what the color of that pixel should be. It only has one input - which is the color that corresponds to the next pixel location. A clarifying diagram is shown in the procedure section. 
